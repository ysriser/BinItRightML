"""Robust fine-tune v1 training for Tier-1 (7-class) classifier."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Robust fine-tune v1")
    p.add_argument(
        "--config",
        type=Path,
...
def main() -> None:
    ...
    g3_data_dir = Path(paths.get("g3_data_dir", "CNN/data/G3_SGData"))
    ...
    run_name = datetime.now().strftime("robust_v1_%Y%m%d_%H%M%S")
