"""Compare baseline vs robust v1 on in-domain data."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Eval baseline vs robust v1")
    p.add_argument(
        "--baseline-model",
        type=Path,
    ...
    p.add_argument(
        "--data-dir",
        type=Path,
        default=Path("CNN/data/G3_SGData"),
    )
    p.add_argument(
        "--reject-config",
        type=Path,
    ...
    p.add_argument(
        "--output-dir",
        type=Path,
        default=Path("CNN/experiments/v1_robust_finetune/outputs"),
    )
