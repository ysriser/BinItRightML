...
Usage:
  python ml/scripts/build_manifest_and_splits.py --config ml/configs/dataset_mix.yaml
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--manifest", type=Path, default=Path("ml/data/manifest_all.csv"))
    p.add_argument("--splits-dir", type=Path, default=Path("ml/data/splits"))
    p.add_argument("--stats", type=Path, default=Path("ml/data/dataset_stats.json"))
    return p.parse_args()
...
def main() -> None:
    ...
    for label in final_labels:
        ...
        for mix in mixes:
            ...
            source_path = Path(sources[source_name]["path"])
            pools[label].setdefault(source_name, [])
            for orig in orig_classes:
                class_dir = source_path / orig
    ...
    for split in splits:
        ...
        for label in final_labels:
            ...
            if shortage > 0:
                # Fallback to any remaining samples from listed sources.
    ...
    if cfg.get("make_links", False):
        ...
        out_dir = Path(links_cfg.get("out_dir", "ml/data/merged"))
        for split in splits:
    ...
    if warnings:
        print(f"Warnings: {len(warnings)} (see dataset_stats.json)")
